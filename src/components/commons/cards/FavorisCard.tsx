...
import { Product, RootState, api_public, categorieDisplay, colors, dislikes, images, likes } from '../../../libs'
import { useNavigation } from '@react-navigation/native'
import { useDispatch, useSelector } from 'react-redux'


const FavorisCard: FC<{ product: Product }> = ({ product }) => {
        ...
        <View style={styles.container}>
            ...
            <View style={styles.cardinfo}>
                <TouchableOpacity onPress={handleDetail} style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <View>
                        <Text style={styles.title}>{product?.title?.slice(0, 18)}{product?.title?.length > 18 && "..."}</Text>
